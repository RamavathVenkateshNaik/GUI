...
# Separate directory array for all_charge.csv files
all_charge_directories = [
    "/home/sujith/Documents/n1a1data/17L/all_charge.csv",
    "/home/sujith/Documents/n1a1data/23L/all_charge.csv",
    "/home/sujith/Documents/n1a1data/31L/all_charge.csv"
]

# Temperature values and layer configurations
temperatures = [15, 50, 77, 100, 150, 200, 250, 300]
layers = [17, 23, 31, 37, 53, 75]
...
test_points = [
    ...
    {'voltage': 2.6710, 'temperature': 150, 'layers': 31},
    {'voltage': 2.1925, 'temperature': 150, 'layers': 37},
    {'voltage': 2.4796, 'temperature': 150, 'layers': 37},
    {'voltage': 2.6710, 'temperature': 150, 'layers': 37}
]
...
# Modified predict_and_plot_example function with user input
def predict_and_plot_example():
    """Interactive prediction and plotting function"""
    
    while True:
        try:
            print(f"\n{'='*80}")
            print("INTERACTIVE PHYSICAL MODEL PREDICTION")
            print(f"{'='*80}")
            
            # First, let user select directory for all_charge.csv
            print("Available all_charge.csv files:")
            available_dirs = []
            for i, all_charge_path in enumerate(all_charge_directories):
                if os.path.exists(all_charge_path):
                    available_dirs.append(all_charge_path)
                    print(f"{i}: {all_charge_path}")
            
            if not available_dirs:
                print("No all_charge.csv files found!")
                break
            
            dir_choice = int(input(f"Enter file number (0-{len(available_dirs)-1}): "))
            if dir_choice < 0 or dir_choice >= len(available_dirs):
                print("Invalid file number!")
                continue
            
            selected_all_charge_path = available_dirs[dir_choice]
            
            # Load all_charge.csv and show columns
            all_charge_data = pd.read_csv(selected_all_charge_path, header=None)
            print(f"\nLoaded all_charge.csv from: {selected_all_charge_path}")
            print(f"Available columns: {list(all_charge_data.columns)}")
            print(f"Data shape: {all_charge_data.shape}")
            
            # Let user select column
            col_choice = int(input(f"Enter column number (0-{all_charge_data.shape[1]-1}): "))
            if col_choice < 0 or col_choice >= all_charge_data.shape[1]:
                print("Invalid column number!")
                continue
            
            # Get n_max as the maximum value from selected column
            user_n_max = all_charge_data[col_choice].max()
            selected_column_data = all_charge_data[col_choice].values
            print(f"Selected column {col_choice} max value (n_max): {user_n_max}")
            
            # Get user input for other parameters
            user_voltage = float(input("Enter voltage: "))
            ...
            # Plot comparison using the same column from all_charge.csv as original data
            plt.figure(figsize=(12, 6))
            
            # Create x values for the original data (assuming same number of points as x_values)
            if len(selected_column_data) == len(x_values):
                original_x = x_values
            else:
                # If different lengths, create x values based on the original data length
                original_tsi, original_x = calculate_tsi_and_x(len(selected_column_data))
            
            plt.plot(original_x, selected_column_data, 'o-', label=f'Original Data (Column {col_choice})', color='black', markersize=4)
            ...
            print(f"  n_max: {user_n_max} (max from all_charge.csv column {col_choice})")
...
# Run interactive prediction and plotting
